...
public class MySqlAdapter implements StorageAdapter {
    ...
    public LogStorageTypeEnum getStorageType() {
        return LogStorageTypeEnum.MYSQL;
    }
    ...
    public UnifiedLogResult query(UnifiedLogQuery query, StorageInfo storageInfo, List<String> keyList) {
        ...
        try {
            ...
            long total = executeCountQuery(dataSource, countSql);
            ...
            return UnifiedLogResult.builder()
                    ...
                    .hasMore(query.getOffset() + logDataList.size() < total)
                    .queryTimeMs(System.currentTimeMillis() - startTime)
                    .storageType(LogStorageTypeEnum.MYSQL.getCode())
                    .debugInfo(sql)
                    .build();
        } catch (Exception e) {
            log.error("MySQL query error, query: {}", query, e);
            return UnifiedLogResult.empty();
        }
    }
    ...
    public UnifiedLogResult queryContext(LogContextQuery contextQuery, StorageInfo storageInfo, List<String> keyList) {
        try {
            ...
            int type = contextQuery.getType();

            // type: 0-both, 1-after, 2-before
            if (type == 0 || type == 2) {
                // Query before
                String beforeSql = buildContextSql(contextQuery, storageInfo.getIndexName(), false, pageSize / 2);
                List<Map<String, Object>> beforeResults = executeQuery(dataSource, beforeSql);
                Collections.reverse(beforeResults);
                logDataList.addAll(transformResults(beforeResults, keyList));
            }

            if (type == 0 || type == 1) {
                // Query after
                String afterSql = buildContextSql(contextQuery, storageInfo.getIndexName(), true, pageSize / 2);
                List<Map<String, Object>> afterResults = executeQuery(dataSource, afterSql);
                logDataList.addAll(transformResults(afterResults, keyList));
            }
            ...
            log.error("MySQL context query error, contextQuery: {}", contextQuery, e);
            return UnifiedLogResult.empty();
        }
    }
    ...
    public AggregationResult aggregate(UnifiedLogQuery query, StorageInfo storageInfo) {
        ...
        try {
            ...
            String statSql = buildStatisticsSql(query, storageInfo.getIndexName());
            log.info("MySQL stat SQL: {}", statSql);
            ...
            log.error("MySQL aggregation error, query: {}", query, e);
            return new AggregationResult();
        }
    }
    ...
    public long count(UnifiedLogQuery query, StorageInfo storageInfo) {
        try {
            ...
            String countSql = buildCountSql(query, storageInfo.getIndexName());
            return executeCountQuery(dataSource, countSql);
        } catch (Exception e) {
            log.error("MySQL count error, query: {}", query, e);
            return 0L;
        }
    }
    ...
    private String buildContextSql(LogContextQuery contextQuery, String tableName, boolean after, int limit) {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT * FROM ").append(tableName);
        sql.append(" WHERE logip = '").append(escapeValue(contextQuery.getIp())).append("'");
        sql.append(" AND filename = '").append(escapeValue(contextQuery.getFileName())).append("'");

        if (after) {
            sql.append(" AND (timestamp > ").append(contextQuery.getTimestamp());
            sql.append(" OR (timestamp = ").append(contextQuery.getTimestamp());
            sql.append(" AND linenumber > ").append(contextQuery.getLineNumber()).append("))");
            sql.append(" ORDER BY timestamp ASC, linenumber ASC");
        } else {
            sql.append(" AND (timestamp < ").append(contextQuery.getTimestamp());
            sql.append(" OR (timestamp = ").append(contextQuery.getTimestamp());
            sql.append(" AND linenumber < ").append(contextQuery.getLineNumber()).append("))");
            sql.append(" ORDER BY timestamp DESC, linenumber DESC");
        }
    ...
    private String buildStatisticsSql(UnifiedLogQuery query, String tableName) {
    ...
    private long executeCountQuery(DataSource dataSource, String sql) throws SQLException {
        try (Connection conn = dataSource.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
            if (rs.next()) {
